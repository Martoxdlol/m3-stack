    ...
    const schema = produceSchema(schemaPrisma, (builder) => {
        for (const table in tables) {
            ...
            for (const field in fields) {
                ...
                builder.model(modelName).field(field, getType(attr.type, !attr?.required, attr?.bigint || false))
                if (attr.unique) {
                ...
                if (attr.references) {
                    builder
                        .model(modelName)
                        .field(`${attr.references.model.toLowerCase()}`, capitalizeFirstLetter(attr.references.model))
                        .attribute(
